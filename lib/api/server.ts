...
async function parseResponse<T>(res: Response): Promise<SuccessEnvelope<T>> {
  ...
  try {
    ...
    throw new TelemetApiError("parse_error", `Failed to parse response (HTTP ${res.status})`, res.status);
  }
  if (!body.ok) {
    throw new TelemetApiError(body.error.code, body.error.message, res.status, body.request_id);
  }
  return body;
}
